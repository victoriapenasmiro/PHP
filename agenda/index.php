    ...
    <body>
        ...
        if (filter_input(INPUT_GET, "submit")) {
            ...
            if (empty($nombre)) {
                ...
                } else if (!empty($telefono) && strlen($telefono) === 9) {

                    //si el nombre no existe en la agenda y se ha indicado un teléfono, lo añado
                    $agenda = filter_input(INPUT_GET, "nombre") . ":" . filter_input(INPUT_GET, "telefono") . "," . filter_input(INPUT_GET, "agenda");
                } else {
                    echo "<h4 style='color:red;'>¡ATENCIÓN!</h4>"
                    . "<ul><li>Para añadir un nuevo contacto, por favor indica un teléfono de 9 dígitos.</li>"
        ...
        function checkNombre($nombre, $tlf) {
            ...
            while ($i < count($contactos)) {
                ...
                } else if ($contacto[0] === $nombre) {

                    checkTlf($tlf, $contacto);
         ...
         * 
         * @param type $tlf nuevo telefono añadido
         * @param type $contacto nombre : tlf
         */
        function checkTlf($tlf, &$contacto) {
            if (strlen($tlf) === 9) {
                //seteo el valor del tlf de este contacto
                $contacto[1] = $tlf;
            } else {
                echo "<h4 style='color:red;'>¡ATENCIÓN!</h4>"
                . "<p>Por favor indica un teléfono de 9 dígitos.</p>";
            }
        }
        ?>
